...
ComBat <- function(expression_xls, sample_info_file, type='txt', write=T, covariates='all', numeric.covariates=NULL, par.prior=T, filter=FALSE, skip=0, prior.plots=T, confounded=FALSE, pval_cut=.2, pct=FALSE){
    ...
    #debug: expression_xls='dataExample1.txt'; sample_info_file='sampleInfoExample1.txt'; type='txt'; write=T; covariates='all'; par.prior=T; filter=F; skip=0; prior.plots=T; numeric.covariates=NULL;confounded=TRUE
	#debug: setwd("~/Desktop/projects/BatchConfound/pathway/"); source("../ComBat_numerical_confound_multibatch.R")
    #debug: expression_xls='pathways.RASPI3K.txt'; sample_info_file='sample_info.txt'; type='txt'; write=T; covariates='all'; par.prior=T; filter=F; skip=1; prior.plots=T; numeric.covariates=NULL;confounded=FALSE
	...
	##Find Priors --- Use only rank-invariant subset if confounded=TRUE
        if(confounded){
          cat("Applying mixture model to separate confounded batch/treatment effects...",'\n')
          ### mixture ###
         gamma.bar <- NULL
		 eta.bar <- NULL
		 x.hat <- NULL
		 t2 <- NULL
		 r2 <- NULL
		 for (j in 1:nrow(gamma.hat)){
		  m1 <- gamma.hat[j,]
		  
		  p<-.5
		  lu <- quantile(m1,c(.25,.75))
		  mid <- m1>lu[1] & m1<lu[2]
		  mu1 <- mean(m1[mid])
		  mu2 <- mean(m1[!mid])
		  s1 <- var(m1[mid])
		  ...
		  while (conv>.01){
			# E-step
			gam <- p*dnorm(m1,mu1+mu2,sqrt(s1+s2))/ (p*dnorm(m1,mu1+mu2,sqrt(s1+s2))+(1-p)*dnorm(m1,mu1,sqrt(s1)))
			# M-step
			p <- mean(gam)
			mu1 <- ((s1+s2)*sum((1-gam)*m1)+s1*sum(gam*(m1-mu2)))/((s1+s2)*sum(1-gam)+s1*sum(gam))
			mu2 <- sum(gam*(m1-mu1))/sum(gam)
			loglik <- function(params,mu1,mu2,m1,gam){sum(1-gam)*log(params[1])+sum(gam)*log(params[1]+params[2])+sum((1-gam)*(m1-mu1)^2)/params[1]+sum(gam*(m1-mu1-mu2)^2)/(params[1]+params[2])}
			tmps <- optim(c(1,1),loglik,mu1=mu1, mu2=mu2, m1=m1,gam=gam,lower=c(0.0001,0.0001),upper=c(3,3),method="L-BFGS-B")
			s1 <- tmps$par[1]
			s2 <- tmps$par[2]
			conv <- max(abs((c(mu1,mu2,s1,s2)-old)/old))
			old <- c(mu1,mu2,s1,s2)
			#cat(conv,p,mu1,s1,mu2,s2,'\n')
			}
		  cat("Estimated proportion of batch-only probes for this batch:",round(p,4),'\n')
		  gamma.bar <- c(gamma.bar,mu1)
		  eta.bar <- c(eta.bar,mu2)
		  x.hat <- rbind(x.hat,gam) 
		  t2 <- c(t2,s1)
		  r2 <- c(r2,s2)
		 }
	...
	if (confounded & !par.prior){par.prior=TRUE; cat("Warning: There is no non-parametric method for confounded batches, changing par.prior=TRUE")}
	
	gamma.star <- delta.star <- NULL
    if(confounded){
     
    cat("Finding confounded parametric adjustments\n")
		for (i in 1:n.batch){
                     if(length(batches[[i]])==1){
 						tmp <- it.sol.confounded(gamma.hat[i,],1,1,x.hat[i,],gamma.bar[i],eta.bar[i],t2[i],r2[i])                    	
						gamma.star <- rbind(gamma.star,tmp[1,])
                        delta.star <- rbind(delta.star,rep(1,nrow(s.data)))
                     }else{
						tmp <- it.sol.confounded(gamma.hat[i,],1,1,x.hat[i,],gamma.bar[i],eta.bar[i],t2[i],r2[i])                    	
						gamma.star <- rbind(gamma.star,tmp[1,])
			temp <- it.sol(s.data[,batches[[i]]],gamma.star[i,],delta.hat[i,],gamma.bar[i],t2[i],a.prior[i],b.prior[i])   ### fix this!!!
...
#it.sol.confounded(gamma.hat[i,],1,1,x.hat[i,],gamma.bar[i],eta.bar[i],t2[i],r2[i])                    	
it.sol.confounded  <- function(ghat,sig,n,gam,g.bar,e.bar,t2,r2,stop=.05){
	g.old <- g.bar
	e.old <- e.bar
	conv <- 1
	count <- 0
	while(conv>stop){
		g.new <- postmean(gam*(ghat-e.old)+(1-gam)*ghat,g.bar,n,sig,t2)
		e.new <- postmean(gam*(ghat-g.new),e.bar,gam*n,sig,r2)
		conv <- max(abs(g.new-g.old)/g.old,abs(e.new-e.old)/e.old)
		g.old <- g.new
		e.old <- e.new
		count <- count+1
                cat(count, conv, g.new[1],e.new[1],'\n')
		}
	adjust <- rbind(g.new, e.new)
	rownames(adjust) <- c("g.star","e.star")
	adjust
	}
