...
# define a random/shuffled set of labels for each category - gender, smoking, cancer
SHUFFLED=FALSE
if(SHUFFLED){
  tempShuffle <- sample(copdESet$FinalCaDXc)
  while(chisq.test(as.numeric(tempShuffle), as.numeric(copdESet$FinalCaDXc))$p.value < 0.9){
    tempShuffle <- sample(copdESet$FinalCaDXc)
  }
}
