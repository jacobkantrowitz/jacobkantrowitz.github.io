...
train.class <- class2Binary(copdESet.train$FinalCaDXc,case=1,control=0)
test.class <- class2Binary(copdESet.test$FinalCaDXc,case=1,control=0)
...
for(i in 1:nrow(index)) {
  ...
  if(min.change <=1 ) {
    ...
    if(ff.m=="cor"){
      cor.p <- 0.05
      l <- computeCorrelation(x=nasal.train,y=bronch.all) 
      ff.eset <- nasal.train[l$p.value<cor.p,]
      ff.genes <- row.names(exprs(ff.eset))
    }
    
    # nasal gsea 200 leading edge
    if(ff.m=="gsea200"){
      
      # ensuring that we start with a clean slate each time through this loop
      if(file.exists("nasal_up_gene_set.gmx")){
        system("rm nasal_up_gene_set.gmx")
      }
      if(file.exists("nasal_down_gene_set.gmx")){
        system("rm nasal_down_gene_set.gmx")
      }
      if(file.exists("bronch_ranked_list.rnk")){
        system("rm bronch_ranked_list.rnk")
      } 
      
      # creating a unique gsea output directory
      gsea.dir <- "/protected/projects/pulmarray/Biollegro/root/results/gsea"
      if(!file.exists(paste("mkdir ",gsea.dir,"/",prefix,sep=""))){
        system(paste("mkdir ",gsea.dir,"/",prefix,sep=""))
      }
      
      # defining the output labels for the two gsea runs
      upreg.label <- paste("gsea_up_iter_",iteration,"_m200",sep="")
      downreg.label <- paste("gsea_down_iter_",iteration,"_m200",sep="")
      
      # running GSEA with only those samples with RIN > 4
      bronch.eset <- bronch.all#[,bronch.all$RIN>4]
      nasal.eset <- nasal.train#[,nasal.all$RIN>4]
      
      # removing bronchial samples with missing data
      missing.data <- unique(c(which(is.na(bronch.eset$AGE)),which(is.na(bronch.eset$CANCER))))
      bronch.eset <- bronch.eset[,-missing.data]
      bronch.eset <- bronch.eset[,!c(bronch.eset$BARCODE%in%nasal.test$BARCODE)]
      
      # also only using the overlapping samples
      bronch.eset <- bronch.eset[,bronch.eset$BARCODE%in%nasal.eset$BARCODE]
      nasal.eset <- nasal.eset[,nasal.eset$BARCODE%in%bronch.eset$BARCODE]
      
      # create the bronch gene sets
      bronch.mod <- model.matrix(~1+SMK+RLE+AGE+GENDER+CANCER,data=pData(bronch.eset))
      bronch.fit <- fitLM(eset=bronch.eset,mod=bronch.mod)
      gs.up <- head(names(bronch.fit$fit.t.sorted),200)
      gs.down <- tail(names(bronch.fit$fit.t.sorted),200)
      writeGeneSet(genes=gs.up,name="bronch_up_200",outfile="bronch_up_gene_set.gmx")
      writeGeneSet(genes=gs.down,name="bronch_down_200",outfile="bronch_down_gene_set.gmx")
      
      # creating the nasal ranked lists
      nasal.mod <- model.matrix(~1+SMK+RLE+AGE+GENDER+CANCER,data=pData(nasal.eset))
      nasal.fit <- fitLM(eset=nasal.eset,mod=nasal.mod)
      rl.genes <- names(nasal.fit$fit.t.sorted)
      rl.ranks <- as.vector(nasal.fit$fit.t.sorted)
      writeRankedList(genes=rl.genes,ranks=rl.ranks,outfile="nasal_ranked_list.rnk")
      
      # running a gsea pre-ranked analysis
      nasal.up.le <- gseaPreranked(gmx.file="bronch_up_gene_set.gmx",rnk.file="nasal_ranked_list.rnk",output.label=upreg.label,output.dir=gsea.dir,keep.files=T) #getwd()
      nasal.down.le <- gseaPreranked(gmx.file="bronch_down_gene_set.gmx",rnk.file="nasal_ranked_list.rnk",output.label=downreg.label,output.dir=gsea.dir,keep.files=T) #getwd()
      
      # subsetting the expression matrix to only contain the leading edge
      ff.eset <- nasal.train[as.character(c(nasal.up.le,nasal.down.le)),]
      ff.genes <- row.names(exprs(ff.eset))
      
      cat(paste("Number of genes in leading edge: ",length(ff.genes),"\n",sep=""))
