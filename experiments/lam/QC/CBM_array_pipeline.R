...
output.path <- "/protected/projects/pulmarray/Allegro/COPD_Cancer/experiments/lam/"
output.prefix <- "LamCOPD_Validate_2014-12-01"

# Change Brainarray mapping and version as needed
brainarray.mapping <- "entrezg"; # could also be "ensg", "refseq", etc.
brainarray.version <- "16.0.0" # e.g., "14.0.0"

# Change the CEL file path as needed
celfile.path <- "/unprotected/projects/pulmarray/CEL/Lam_Airway_COPD_Samples_238/"
...
sample.annotation.filename <- "/restricted/projectnb/pulmarray/LinGA_protected/Allegro/COPD_Cancer/experiments/lam/lam_phenoData4.txt"
...
colnames(QC.summary) <- paste("Median", colnames(QC.summary));
