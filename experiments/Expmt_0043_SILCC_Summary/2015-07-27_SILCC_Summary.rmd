...
### GSVA Score Comparisons
```{r gsvaComparisons}
t.test(exprs(gsvaResults$es.obs)[lm_dn, noCopd.noCancer], exprs(gsvaResults$es.obs)[lm_dn, copd.noCancer])

t.test(exprs(gsvaResults$es.obs)[lm_up, noCopd.noCancer], exprs(gsvaResults$es.obs)[lm_up, copd.noCancer])

t.test(exprs(gsvaResults$es.obs)[erSt, noCopd.noCancer], exprs(gsvaResults$es.obs)[erSt, copd.noCancer])

t.test(exprs(gsvaResults$es.obs)[cil, noCopd.noCancer], exprs(gsvaResults$es.obs)[cil, copd.noCancer])

t.test(exprs(gsvaResults$es.obs)[canUp, noCopd.noCancer], exprs(gsvaResults$es.obs)[canUp, copd.noCancer])

t.test(exprs(gsvaResults$es.obs)[canDn, noCopd.noCancer], exprs(gsvaResults$es.obs)[canDn, copd.noCancer])

gsvaLM <- lmFitWrapper(gsvaResults$es.obs, c("COPD2_R7", "FinalCaDXc", "COPD2_R7*FinalCaDXc"), 3, "fdr", 0.25)
...
genes <- genes[which(!is.na(match(genes, featureNames(esetCleanFormers))))]
inds <- match(genes, featureNames(esetClean))
target <- removeFactorLevel(esetClean[inds, ], "AllegroCorticosteroidsc", "DK")
erModel <- lmFitWrapper(target, c("FinalCaDXc", "COPD2_R7", "GENDERc", "AGEcalc", "RIN", "AllegroCorticosteroidsc","RATIOc", "RATIOc*FinalCaDXc"), 1, "none", 0.1, "ERStressModel")

clabels <- cbind(copdca_colors_colors[as.character(target$indicator)], smoking_colors[as.character(target$GENDERc)])
h0 = heatmap3(exprs(target)[twoCancerCurSmkCOPD$inds, ], col=bluered,
              hclustfun=function(d) hclust(d, method="ward.D"),
              keep.dendro=TRUE, ColSideColors=clabels,
              labCol="", col.clustering="semisupervised",
