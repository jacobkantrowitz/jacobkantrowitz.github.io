...
heatmap3(exprs(target)[tempModel$inds[tempClusters==1], ], col=bluered,
         hclustfun=function(d) hclust(d, method="ward.D"),
         col.clustering=target$indicator, ColSideColors=clabels,
         main=paste(tempModel$hMapName, "\ncluster1"), labCol="", keep.dendro=TRUE)

heatmap3(exprs(target)[tempModel$inds[tempClusters==2], ], col=bluered,
         hclustfun=function(d) hclust(d, method="ward.D"),
         col.clustering=target$indicator, ColSideColors=clabels,
         main=paste(tempModel$hMapName, "\ncluster2"), labCol="", keep.dendro=TRUE)
...
if(SAVE){
  ...
  write.table(tempModel$geneSymbols[tempClusters==1], file=genFileName(paste(tName, "c1.txt", sep="")), quote=FALSE, row.names=FALSE, col.names=FALSE)
  
  # save cluster 2 gene list
  write.table(tempModel$geneSymbols[tempClusters==2], file=genFileName(paste(tName, "c2.txt", sep="")), quote=FALSE, row.names=FALSE, col.names=FALSE)
...
tName = "intrxFormer"
tempModel <- lmFitWrapper(target, c("COPD2_R7", "FinalCaDXc", "AGEcalc", "RIN", "GENDERc", "COPD2_R7*FinalCaDXc"), 6, "none", 0.01, "Interaction Model 1 Among Former Smokers")
...
tName = "intrxCurrent_ICS"
tempModel <- lmFitWrapper(target, c("COPD2_R7", "FinalCaDXc", "AGEcalc", "RIN", "GENDERc", "AllegroCorticosteroidsc", "COPD2_R7*FinalCaDXc"), 7, "none", 0.05, "Interaction Model 2 +ICS Among Current Smokers")
...
## 3 v 1 Group Analysis - this is statistically bad but do it anyways    
### Among All Patients  
```{r tvoAll}

tvoDir = "ThreeVOne"
if(!length(grep(tvoDir, list.files()))){
  dir.create(tvoDir)
  print(paste(tvoDir, "directory created", Sys.Date()))
  } else {
    print(paste(tvoDir, "already created"))
    }

target = medianFilter(removeFactorLevel(esetClean, "AllegroCorticosteroidsc", "DK"))
target$group = as.numeric(as.character(target$indicator))
target$group[target$group < 4] = 1
target$group = as.factor(target$group)

tName = "tvoAll"

tvoM0 = lmFitWrapper(target, c("group"), 1, "fdr", 0.05, "3 v 1 All Patients")

clabels = cbind(copdca_colors[as.character(target$indicator)], smoking_colors[as.character(target$SMKc)])

heatmap3(exprs(target)[tvoM0$inds, ], col=bluered,
         hclustfun=function(d) hclust(d, method="ward.D"),
         col.clustering=target$indicator, ColSideColors=clabels,
         main=tvoM0$hMapName, labCol="", keep.dendro=TRUE)

tvoM1 = lmFitWrapper(target, c("group", "SMKc"), 1, "fdr", 0.05, "3 v 1 All Patients + SMK")

heatmap3(exprs(target)[tvoM1$inds, ], col=bluered,
         hclustfun=function(d) hclust(d, method="ward.D"),
         col.clustering=target$indicator, ColSideColors=clabels,
         main=tvoM1$hMapName, labCol="", keep.dendro=TRUE)

tvoM2 = lmFitWrapper(target, c("group", "SMKc", "group*SMKc"), 1, "fdr", 0.25, "3 v 1 All Patients + SMK Intrx")

tempHMap = heatmap3(exprs(target)[tvoM2$inds, ], col=bluered,
                    hclustfun=function(d) hclust(d, method="ward.D"),
                    col.clustering=target$indicator, ColSideColors=clabels,
                    main=tvoM2$hMapName, labCol="", keep.dendro=TRUE)

tempClusters <- cutree(as.hclust(tempHMap$Rowv), k=2)

if(SAVE){

  fName = file.path(tvoDir, genFileName(tName))
  # save entire gene list
  write.table(tvoM1$geneSymbols, file=paste(fName, ".txt", sep=""), quote=FALSE, row.names=FALSE, col.names=FALSE)
  # save cluster 1 gene list
  write.table(tvoM1$geneSymbols[tempClusters==1], file=paste(fName, "c1.txt", sep=""), quote=FALSE, row.names=FALSE, col.names=FALSE)
  
  # save cluster 2 gene list
  write.table(tvoM1$geneSymbols[tempClusters==2], file=paste(fName, "c2.txt", sep=""), quote=FALSE, row.names=FALSE, col.names=FALSE)
...
tvoM1 = lmFitWrapper(target, c("group"), 1, "fdr", 0.05, "3 v 1 Former Smokers")

clabels = cbind(copdca_colors[as.character(target$indicator)], smoking_colors[as.character(target$SMKc)])

tempHMap = heatmap3(exprs(target)[tvoM1$inds, ], col=bluered,
                    hclustfun=function(d) hclust(d, method="ward.D"),
                    col.clustering=target$indicator, ColSideColors=clabels,
                    main=tvoM1$hMapName, labCol="", 
                    labRow=tvoM1$geneSymbols, keep.dendro=TRUE)
...
tvoM1 = lmFitWrapper(target, c("group"), 1, "fdr", 0.25, "3 v 1 Current Smokers")

clabels = cbind(copdca_colors[as.character(target$indicator)])
...
## 3 Model Overlapping Genes Approach  
### Among All Patients  
```{r tripModAll}
SAVE = FALSE
tOverDir = "TripleOverlap"
if(!length(grep(tOverDir, list.files()))){
  dir.create(tOverDir)
  print(paste(tOverDir, "directory created", Sys.Date()))
  } else {
    print(paste(tOverDir, "already created"))
    }

tName = "tripleOverlapAll"
...
clabels = cbind(copdca_colors[as.character(target$indicator)], smoking_colors[as.character(target$SMKc)])
tOhMap = heatmap3(exprs(target)[tripleModAll$intrxInds[[3]], ], col=bluered,
                  hclustfun=function(d) hclust(d, method="ward.D"),
                  col.clustering=target$indicator, ColSideColors=clabels,
                  main="Overlapping interaction genes", labCol="",
                  labRow=tripleModAll$intrxGenes[[3]], keep.dendro=TRUE)
tempClusters <- cutree(as.hclust(tOhMap$Rowv), k=2)

heatmap3(exprs(target)[tripleModAll$intrxInds[[3]][tempClusters==1], ], col=bluered,
                  hclustfun=function(d) hclust(d, method="ward.D"),
                  col.clustering=target$indicator, ColSideColors=clabels,
                  main="Overlapping interaction genes cluster 1", labCol="",
                  labRow=tripleModAll$intrxGenes[[3]][tempClusters==1], keep.dendro=TRUE)

heatmap3(exprs(target)[tripleModAll$intrxInds[[3]][tempClusters==2], ], col=bluered,
                  hclustfun=function(d) hclust(d, method="ward.D"),
                  col.clustering=target$indicator, ColSideColors=clabels,
                  main="Overlapping interaction genes cluster 2", labCol="",
                  labRow=tripleModAll$intrxGenes[[3]][tempClusters==2], keep.dendro=TRUE)

if(SAVE){

  fName = file.path(tOverDir, genFileName(tName))
  # save entire gene list
  write.table(tripleModAll$intrxGenes[[3]], file=paste(fName, ".txt", sep=""), quote=FALSE, row.names=FALSE, col.names=FALSE)
  # save cluster 1 gene list
  write.table(tripleModAll$intrxGenes[[3]][tempClusters==1], file=paste(fName, "c1.txt", sep=""), quote=FALSE, row.names=FALSE, col.names=FALSE)
  
  # save cluster 2 gene list
  write.table(tripleModAll$intrxGenes[[3]][tempClusters==2], file=paste(fName, "c2.txt", sep=""), quote=FALSE, row.names=FALSE, col.names=FALSE)
...
clabels = cbind(copdca_colors[as.character(target$indicator)], smoking_colors[as.character(target$SMKc)])
tOhMap = heatmap3(exprs(target)[tripleModFormers$intrxInds[[3]], ], col=bluered,
                  hclustfun=function(d) hclust(d, method="ward.D"),
                  col.clustering=target$indicator, ColSideColors=clabels,
                  main="Overlapping interaction genes in former smokers", labCol="",
                  labRow=tripleModFormers$intrxGenes[[3]], keep.dendro=TRUE)

tempClusters <- cutree(as.hclust(tOhMap$Rowv), k=2)

heatmap3(exprs(target)[tripleModFormers$intrxInds[[3]][tempClusters==1], ], col=bluered,
                  hclustfun=function(d) hclust(d, method="ward.D"),
                  col.clustering=target$indicator, ColSideColors=clabels,
                  main="Overlapping interaction genes in former smokers cluster 1", labCol="",
                  labRow=tripleModFormers$intrxGenes[[3]][tempClusters==1], keep.dendro=TRUE)

heatmap3(exprs(target)[tripleModFormers$intrxInds[[3]][tempClusters==2], ], col=bluered,
                  hclustfun=function(d) hclust(d, method="ward.D"),
                  col.clustering=target$indicator, ColSideColors=clabels,
                  main="Overlapping interaction genes in former smokers cluster 2", labCol="",
                  labRow=tripleModFormers$intrxGenes[[3]][tempClusters==2], keep.dendro=TRUE)

if(SAVE){

  fName = file.path(tOverDir, genFileName(tName))
  # save entire gene list
  write.table(tripleModFormers$intrxGenes[[3]], file=paste(fName, ".txt", sep=""), quote=FALSE, row.names=FALSE, col.names=FALSE)
  # save cluster 1 gene list
  write.table(tripleModFormers$intrxGenes[[3]][tempClusters==1], file=paste(fName, "c1.txt", sep=""), quote=FALSE, row.names=FALSE, col.names=FALSE)
  
  # save cluster 2 gene list
  write.table(tripleModFormers$intrxGenes[[3]][tempClusters==2], file=paste(fName, "c2.txt", sep=""), quote=FALSE, row.names=FALSE, col.names=FALSE)
...
clabels = cbind(copdca_colors[as.character(target$indicator)], smoking_colors[as.character(target$SMKc)])
tOhMap = heatmap3(exprs(target)[tripleModCurrents$intrxInds[[3]], ], col=bluered,
                  hclustfun=function(d) hclust(d, method="ward.D"),
                  col.clustering=target$indicator, ColSideColors=clabels,
                  main="Overlapping interaction genes in current smokers", labCol="",
                  labRow=tripleModCurrents$intrxGenes[[3]], keep.dendro=TRUE)

tempClusters <- cutree(as.hclust(tOhMap$Rowv), k=2)

heatmap3(exprs(target)[tripleModCurrents$intrxInds[[3]][tempClusters==1], ], col=bluered,
                  hclustfun=function(d) hclust(d, method="ward.D"),
                  col.clustering=target$indicator, ColSideColors=clabels,
                  main="Overlapping interaction genes in current smokers cluster 1", labCol="",
                  labRow=tripleModCurrents$intrxGenes[[3]][tempClusters==1], keep.dendro=TRUE)

heatmap3(exprs(target)[tripleModCurrents$intrxInds[[3]][tempClusters==2], ], col=bluered,
                  hclustfun=function(d) hclust(d, method="ward.D"),
                  col.clustering=target$indicator, ColSideColors=clabels,
                  main="Overlapping interaction genes in current smokers cluster 2", labCol="",
                  labRow=tripleModCurrents$intrxGenes[[3]][tempClusters==2], keep.dendro=TRUE)
...
gsvaResults <- gsva(eset, genesets)
gsvaHMark <- gsva(eset, hMarkList)
gsvaCurated <- gsva(eset, curList)
...
Thought - these types of analyses are not disimilar from and in fact may be required when attempting to interpret an interaction term model. Checking the overlap of those could help elucidate directionality changes.
...
fdr <- 0.25
results <- decideTests(fit2, adjust.method="fdr", p.value=fdr)
vennDiagram(results, main="fdr 0.25")
...
inds = match(genes, fData(target)$Symbol)
inds = inds[!is.na(inds)]
target = target[inds, ]
results = lmFitWrapper(target, c("COPD2_R7"), 1)
fData(target)$Symbol[which(results$results[, 2] > 0)]
fData(target)$Symbol[which(results$results[, 2] < 0)]

target2 = removeFactorLevel(target, "indicator", "2")
target2 = removeFactorLevel(target2, "indicator", "4")
results2 = lmFitWrapper(target2, c("COPD2_R7"), 1)
fData(target2)$Symbol[which(results2$results[, 2] > 0)]
fData(target2)$Symbol[which(results2$results[, 2] < 0)]

target3 = removeFactorLevel(target, "indicator", "2")
target3 = removeFactorLevel(target3, "indicator", "1")
results3 = lmFitWrapper(target3, c("FinalCaDXc"), 1)
fData(target3)$Symbol[which(results3$results[, 2] > 0)]
fData(target3)$Symbol[which(results3$results[, 2] < 0)]
...
# Modeling using genomics correlates
```{r genomicSignaturesModels}
# use the lam signature (PCA or GSVA as a COPD surrogate)
# use one of the smoking genomic signatures
#

```

# Check 98 lam genes in COPD v COPD+Cancer
```{r lamCOPDvCC}
target = esetClean
target = removeFactorLevel(target, "COPD2_R7", "0")
target = target[!is.na(fData(target)$Symbol), ]
featureNames(target) = fData(target)$Symbol

gsvaResults <- gsva(target, genesets)

gsvaTest = lmFitWrapper(gsvaResults$es.obs, c("FinalCaDXc"), 1, "none")
gsvaTest$inds
