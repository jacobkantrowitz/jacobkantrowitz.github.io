...
fullModel1 <- lmFitWrapper(esetF, covariates1, 1, p.value=0.10)
generate_heatmap(fullModel1$inds, fullModel1$eset, tp="FinalCaDXc")
...
clusters9 <- return_cluster(inds9, esetF, n.clusters=2)

generate_heatmap(inds9, esetF, tp="indicator", mn="Model9")
generate_heatmap(inds9[clusters9==1], esetF, tp="indicator", mn="Model9Up")
generate_heatmap(inds9[clusters9==2], esetF, tp="indicator", mn="Model9Down")

save_entrez(inds9[clusters9==1], rownames(esetF), filename="AllOrNoneUP.AGERINPYGENDER.txt")
save_entrez(inds9[clusters9==2], rownames(esetF), filename="AllOrNoneDOWN.AGERINPYGENDER.txt")

# TEST THE 50 UP GENES
testUPCOPDCancervCOPD <- t.test(exprs(esetF)[inds9[clusters9==1], esetF$indicator==3], exprs(esetF)[inds9[clusters9==1], esetF$indicator==4])
testUPCOPDCancervCOPD

testUPCOPDCancervCancer <- t.test(exprs(esetF)[inds9[clusters9==1], esetF$indicator==2], exprs(esetF)[inds9[clusters9==1], esetF$indicator==4])
testUPCOPDCancervCancer

testUPCOPDCancervDiseaseFree <- t.test(exprs(esetF)[inds9[clusters9==1], esetF$indicator==1], exprs(esetF)[inds9[clusters9==1], esetF$indicator==4])
testUPCOPDCancervDiseaseFree

# TEST THE 38 DOWN GENES
testDOWNCOPDCancervCOPD <- t.test(exprs(esetF)[inds9[clusters9==2], esetF$indicator==3], exprs(esetF)[inds9[clusters9==2], esetF$indicator==4])
testDOWNCOPDCancervCOPD

testDOWNCOPDCancervCancer <- t.test(exprs(esetF)[inds9[clusters9==2], esetF$indicator==2], exprs(esetF)[inds9[clusters9==2], esetF$indicator==4])
testDOWNCOPDCancervCancer

testDOWNCOPDCancervDiseaseFree <- t.test(exprs(esetF)[inds9[clusters9==2], esetF$indicator==1], exprs(esetF)[inds9[clusters9==2], esetF$indicator==4])
...
heatmap3(eset[inds11,], col = bluered,
