...
1. Generate COPD signal
  a. output ranked list and check Katie's signature for enrichment
2. Generate cancer signal
  a. output ranked list and check nature medicine signature for enrichment
3. Generate list of overlapping genes for COPD*Cancer interaction, maintaining directionality of t statistic
4. Check overlapping set of genes in nature medicine
  a. separate
...
setwd("/protected/projects/pulmarray/Allegro/COPD_Cancer/experiments/CBM_Seminar/")
...
# BASED ON GRANT'S SUGGESTION (i.e. look at smoking interaction with disease and UPR)
# generate an indicator (8 levels) of copd*cancer*smk
eset.resid$smkindic <- as.numeric(as.character(eset.resid$indicator))
eset.resid$smkindic[eset.resid$SMKc==1] = eset.resid$smkindic[eset.resid$SMKc==1] + 4
eset.resid$smkindic <- as.factor(eset.resid$smkindic)

nms2 <- c("Neither2", "Cancer2", "COPD2", "Both2", "Neither1", "Cancer1", "COPD1", "Both1")
pdf("tripleOverlapBoxPlots_fdr05_040715.pdf")
for(i in 1:length(tripleOver[[1]])){
  boxplot(exprs(eset.resid)[tripleOver[[1]][i], ] ~ eset.resid$smkindic, col=copdca_colors, notch=TRUE, names=nms2, main=intrxGenes[[1]][i])
}
dev.off()
