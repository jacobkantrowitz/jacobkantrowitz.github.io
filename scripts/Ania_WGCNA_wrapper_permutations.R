...
plotSoftThreshold = function(datExpr, powerVector = 1:20, RsquaredCut = 0.90, corFnc = "cor", networkType = "unsigned", nCores=16) {
...
wgcna = function(datExpr, softPower = 6, corFnc="cor", networkType="unsigned", TOMType="unsigned", cutHeight=0.995, minModuleSize=20, 
                 MEDissThres=0.15, saveAdjacency=NULL, saveDissTOM=NULL, saveDendro=NULL, plotModuleHeatmaps=NULL, nCores=16) {
  ...
  if(is.character(saveDendro)) {
    save(geneTree, file=saveDendro)
  }  

  dynamicMods = cutreeDynamic(dendro = geneTree, cutHeight=cutHeight, method="tree", minClusterSize = minModuleSize);
