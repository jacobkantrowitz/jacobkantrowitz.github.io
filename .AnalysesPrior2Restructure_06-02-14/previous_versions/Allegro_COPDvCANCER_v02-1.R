...
return_sva <- function(data_expression, data_phenotype)
{
	...
	mod1 <- model.matrix(~BATCH + RIN + AGE + SMK + GENDER + COPD + CANCER, data=data_phenotype)
	mod0 <- model.matrix(~BATCH + RIN + AGE + SMK + GENDER + CANCER, data=data_phenotype)
...
#sva1 <- return_sva(data_expression, data_phenotype)
#sva2 <- return_sva(data_filtered$cv_mn, data_phenotype)
...
generate_heatmap  <- function(inds, data, phen)
{
	clabels <- cbind(
	...
	#heatmap3(data[inds,], col = bluered, hclustfun=function(d) hclust(d, method="ward"), col.clustering = "semisupervised", ColSideColors = clabels, main = "Figure")
...
#
#pdf(NULL)
##res <- heatmap3(data_to_analyze[inds,], col = bluered, hclustfun=function(d) hclust(d, method="ward"), col.clustering = "semisupervised", ColSideColors = clabels, main = "Figure")
#res <- heatmap3(data_to_analyze[copdca,], col = bluered, keep.dendro=TRUE, col.clustering = "unsupervised", ColSideColors = clabels, main = "Figure")
#dev.off()
#n.clusters <- 6
...
##### RUN SVA
sva_COPD <- return_sva(data_expression, data_phenotype)
...
# switch in results_resid$model_resids for results_intrx$data for heatmap generation
#results_intrx_copy <- results_intrx
#results_intrx_copy$data <- results_resid$model_resids

##################################################################
##################################################################
## Analysis
## Filtering with CV
## set data_to_analyze (14764 genes); set data_phenotype
#data_to_analyze <- data_filtered$cv; data_phenotype <- prepare_data(data_phenotype)
#results_intrx_cvFilt <- analyze_covariate_lists(data_to_analyze, data_phenotype, covs_intrx)
#
## p-value and FDR cutoffs
#cutoffs_intrx_cvFilt_p <- return_cutoff_lists(results_intrx_cvFilt$model_ps, UNCORRECTED)
#cutoffs_intrx_cvFilt_fdr <- return_cutoff_lists(results_intrx_cvFilt$model_fdr, CORRECTED)
#
##################################################################
##################################################################
# Analysis
# Filtering with CV and Mean
# set data_to_analyze (11074 genes); set data_phenotype
#data_to_analyze <- data_filtered$cv_mn; data_phenotype <- prepare_data(data_phenotype)
#results_intrx_cvmnFilt <- analyze_covariate_lists(data_to_analyze, data_phenotype, covs_intrx)
#
## p-value and FDR cutoffs
#cutoffs_intrx_cvmnFilt_p <- return_cutoff_lists(results_intrx_cvmnFilt$model_ps, UNCORRECTED)
#cutoffs_intrx_cvmnFilt_fdr <- return_cutoff_lists(results_intrx_cvmnFilt$model_fdr, CORRECTED)


##################################################################
##################################################################
# Analysis 
# Create a subset of data using results_total_cvFilt
#data_to_analyze <- data_filtered$cv_mn; data_phenotype <- prepare_data(data_phenotype)
#covs_total <- list("BATCH", "RIN", "SMK", "GENDER", "AGE", "COPD", "CANCER")
#results_total_cvmnFilt <- analyze_covariate_lists(data_to_analyze, data_phenotype, covs_total)
#
## p-value and FDR cutoffs
#cutoffs_total_cvmnFilt_p <- return_cutoff_lists(results_total_cvmnFilt$model_ps, UNCORRECTED)
#cutoffs_total_cvmnFilt_fdr <- return_cutoff_lists(results_total_cvmnFilt$model_fdr, CORRECTED)
#
## find genes of potential interest for COPD*CANCER
#copd_ind <- cutoffs_total_cvmnFilt_p[[1]]$COPD1
#cancer_ind <- cutoffs_total_cvmnFilt_p[[1]]$CANCER1
#smk_ind <- cutoffs_total_cvmnFilt_p[[1]]$SMK
#
## these are the indices for the genes to include in the next analysis
#pulm_ind <- unique(union(copd_ind, union(cancer_ind, smk_ind)))
#data_pulm <- keep_genes(pulm_ind, data_expression)
#
## Re-run the analysis using data_pulm and COPD, CANCER, COPD*CANCER
## Can only validly look at COPD*CANCER term because others were used to choose the genes 
#covs_copdca_intrx <- list("COPD", "CANCER", "COPD*CANCER")
#results_intrx_pulm <- analyze_covariate_lists(data_pulm, data_phenotype, covs_copdca_intrx)
#
#cutoffs_intrx_pulm_p <- return_cutoff_lists(results_intrx_pulm$model_p, UNCORRECTED)
#cutoffs_intrx_pulm_fdr <- return_cutoff_lists(results_intrx_pulm$model_fdr, CORRECTED)
#
## generate heatmaps for results_intrx_pulm using data residuals from results_resid
##results_intrx_pulm_copy <- results_intrx_pulm
##results_intrx_pulm_copy$data <- results_resid$model_resid
##generate_heatmaps(results_intrx_pulm_copy)
#
## generate heatmaps with normal data
#generate_heatmaps(results_intrx_pulm)

#################################################################
#################################################################
# Analysis
# Include Surrogate Variables in the model to clean up the noise
# SVA in this case includes all model terms - BATCH, RIN, SMK, AGE, GENDER, CANCER, COPD, CANCER*COPD
# Should make SVA as modular as other functions, allowing specific list of variables to run
#sva1 <- return_sva(data_expression, data_phenotype)
#sva2 <- return_sva(data_filtered$cv_mn, data_phenotype)
# for sva1, ran linear models with all the traits and found those most related
# Batch - none significant, 28 p = 0.07; Rin - sv 1; Cancer - sv 19, 23; Smk - 11, 15
# COPD - 3, 6; Gender - 24; Age - 16
# Lower number surrogate variables are more important, thus COPD seems to be related to 
# a significant amount of variance in the data not captured by the current traits
# if the phenotype data table with the saved surrogates is not loaded:
#load(""data_phenotype_sva.Rda")
#covs_intrx_sv <- list("BATCH", "RIN", "SMK", "GENDER", "AGE", "COPD", "CANCER", "COPD*CANCER", 
#					"copd_sv3", "copd_sv6", "rin_sv1", "smk_sv11", "sv2", "sv4", "sv5")
#results_intrx_sv <- analyze_covariate_lists(data_expression, data_phenotype_sva, covs_intrx_sv)
#
## p-value and FDR cutoffs
#cutoffs_intrx_sv_p <- return_cutoff_lists(results_intrx_sv$model_ps, UNCORRECTED)
#cutoffs_intrx_sv_fdr <- return_cutoff_lists(results_intrx_sv$model_fdr, CORRECTED)
#
#
#
## Analysis surrogate variables in place of COPD
#covs_intrx_sv2 <- list("BATCH", "RIN", "SMK", "GENDER", "AGE", "CANCER", "copd_sv3*CANCER", 
#					"copd_sv3", "copd_sv6", "rin_sv1", "smk_sv11", "sv2", "sv4", "sv5")
#results_intrx_sv2 <- analyze_covariate_lists(data_expression, data_phenotype_sva, covs_intrx_sv2)
#
## p-value and FDR cutoffs
#cutoffs_intrx_sv2_p <- return_cutoff_lists(results_intrx_sv2$model_ps, UNCORRECTED)
#cutoffs_intrx_sv2_fdr <- return_cutoff_lists(results_intrx_sv2$model_fdr, CORRECTED)
...
#sva_lm_ps <- matrix(nrow=sva1$n.sv, ncol=dim(data_phenotype)[2])
#for(sva_var in 1:sva1$n.sv)
#{
#	for(trait in 1:length(data_phenotype))
#	{
#		mod1 <- lm(sva1$sv[,sva_var] ~ data_phenotype[,trait])
