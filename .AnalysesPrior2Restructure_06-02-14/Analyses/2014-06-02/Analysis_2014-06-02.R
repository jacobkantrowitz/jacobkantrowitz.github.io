...
## NEW CODE FOR 2014-06-02 STARTING HERE
## RAN EVERYTHING ABOVE AS IS FROM 05-17-14
## WOULD LIKE TO RUN COPD MODELS FOR COPD+EM+BR, COPDR7, COPDLLN, FEV1/FVC

# need to edit RATIOc to be a numeric, not a factor
covars.copd1 <- list(list(list("RATIOc"), TRUE),
					list(list("RATIOc", "FinalCaDXc", "RATIOc*FinalCaDXc"), TRUE),
					list(list("COPD2_R7"), TRUE),
					list(list("COPD2_R7", "FinalCaDXc", "COPD2_R7*FinalCaDXc"), TRUE))
					
# use noNA.copdR7

covars.copd2 <- list(list(list("COPD_EM_Bronchitisc"), TRUE),
					list(list("COPD_EM_Bronchitisc", "FinalCaDXc", "COPD_EM_Bronchitisc*FinalCaDXc"), TRUE))

# use noNA.all

covars.copd3 <- list(list(list("COPD2LLN"), TRUE),
					list(list("COPD2LLN", "FinalCaDXc", "COPD2LLN*FinalCaDXc"), TRUE))

# use noNA.copdLLN[[2]]

remove.COPDLLN.NA <- which(is.na(mergeAnnotation$COPD2LLN))

noNA.copdLLN <- remove_patients(remove.COPDLLN.NA, data_expression, mergeAnnotation)
noNA.copdLLN[[2]]$COPD2LLN <- as.factor(noNA.copdLLN[[2]]$COPD2LLN)

remove.COPDLLN.NA2 <- which(noNA.copdLLN[[2]]$FinalCaDXc=="DK")
noNA.copdLLN2 <- remove_patients(remove.COPDLLN.NA2, noNA.copdLLN[[1]], noNA.copdLLN[[2]])

noNA.copdLLN2[[2]]$FinalCaDXc <- factor(noNA.copdLLN2[[2]]$FinalCaDXc, levels=c("0", "1"))


noNA.copdR7[[2]]$RATIOc <- as.numeric(noNA.copdR7[[2]]$RATIOc)
noNA.copdR7[[2]]$FinalCaDXc <- factor(noNA.copdR7[[2]]$FinalCaDXc, levels=c("1", "0"))
noNA.copdR7[[2]]$COPD2_R7 <- factor(noNA.copdR7[[2]]$COPD2_R7, levels=c("1", "0"))

noNA.all[[2]]$COPD_EM_Bronchitisc <- factor(noNA.all[[2]]$COPD_EM_Bronchitisc, levels=c("1", "0"))

# analysis
results.copd1 <- analyze_lists(covars.copd1, noNA.copdR7[[1]], noNA.copdR7[[2]])

results.copd2 <- analyze_lists(covars.copd2, noNA.all[[1]], noNA.all[[2]])

results.copd3 <- analyze_lists(covars.copd3, noNA.copdLLN2[[1]], noNA.copdLLN2[[2]])



# need a good naming convention for saved files
# files that haven't been reviewed should start with NR
# then yyyy-mo-dd.genesXXX.term.p/fdr.cutoff



# for results.copd1
save.genes(results.copd1[[4]]$cutoffs_fdr[[3]]$'COPD2_R70:FinalCaDXc0', noNA.copdR7[[1]], noNA.copdR7[[2]], "NR.2014-06-02.genes279.copdR7cancer.fdr.0.1")
save.genes(results.copd1[[2]]$cutoffs_p[[2]]$'RATIOc:FinalCaDXc0', noNA.copdR7[[1]], noNA.copdR7[[2]], "NR.2014-06-02.genes228.ratioCancer.p.0.01")


# for results.copd2

ind0 <- which(noNA.all[[2]]$COPD_EM_Bronchitisc==0 & noNA.all[[2]]$FinalCaDXc==0)
ind1 <- which(noNA.all[[2]]$COPD_EM_Bronchitisc==1 & noNA.all[[2]]$FinalCaDXc==0)
ind2 <- which(noNA.all[[2]]$COPD_EM_Bronchitisc==0 & noNA.all[[2]]$FinalCaDXc==1)
ind3 <- which(noNA.all[[2]]$COPD_EM_Bronchitisc==1 & noNA.all[[2]]$FinalCaDXc==1)


# need to fix the code for generating the heatmaps; currently uses the indicator function, which needs to be replaced with new info
# will replace cClus variable in save.genes function with new indicator function
# generate new indicator function
indicator.new <- numeric(nrow(noNA.all[[2]]))
indicator.new[ind0] <- 0
indicator.new[ind1] <- 1
indicator.new[ind2] <- 2
indicator.new[ind3] <- 3
noNA.all[[2]]$indicator <- as.factor(indicator.new)

save.genes(results.copd2[[2]]$cutoffs_p[[2]]$'COPD_EM_Bronchitisc0:FinalCaDXc1', noNA.all[[1]], noNA.all[[2]], "NR.2014-06-02.genes403.copdEMBRcancer.p.0.01")

# for results.copd3

ind0 <- which(noNA.copdLLN2[[2]]$COPD2LLN==0 & noNA.copdLLN2[[2]]$FinalCaDXc==0)
ind1 <- which(noNA.copdLLN2[[2]]$COPD2LLN==1 & noNA.copdLLN2[[2]]$FinalCaDXc==0)
ind2 <- which(noNA.copdLLN2[[2]]$COPD2LLN==0 & noNA.copdLLN2[[2]]$FinalCaDXc==1)
ind3 <- which(noNA.copdLLN2[[2]]$COPD2LLN==1 & noNA.copdLLN2[[2]]$FinalCaDXc==1)


# need to fix the code for generating the heatmaps; currently uses the indicator function, which needs to be replaced with new info
# will replace cClus variable in save.genes function with new indicator function
# generate new indicator function
indicator.new <- numeric(nrow(noNA.copdLLN2[[2]]))
indicator.new[ind0] <- 0
indicator.new[ind1] <- 1
indicator.new[ind2] <- 2
indicator.new[ind3] <- 3
noNA.copdLLN2[[2]]$indicator <- as.factor(indicator.new)


save.genes(results.copd3[[2]]$cutoffs_fdr[[3]]$'COPD2LLN1:FinalCaDXc1', noNA.copdLLN2[[1]], noNA.copdLLN2[[2]], "NR.2014-06-02.genes268.copdLLNcancer.fdr.0.1")



# comparing the LLN and R7 significant interaction genes
lln1 <- results.copd3[[2]]$cutoffs_fdr[[3]]$'COPD2LLN1:FinalCaDXc1'
lln1.names <- rownames(noNA.copdLLN2[[1]])[lln1]
r71 <- results.copd1[[4]]$cutoffs_fdr[[3]]$'COPD2_R70:FinalCaDXc0'
r71.names <- rownames(noNA.copdR7[[1]])[r71]

#158 genes match from LLN and R7
