plot.gsea <- function(gene.rank.file, gene.set.files, low="blue",
                      ...
                      edge.color="green", edge.highlight=F, all.color="black",
                      gene.sets.results=NULL) {
  ...
  for (i in length(gene.set.files):1) {
    ...
    ## Plot gene set label
    if(plot.labels) {
      #	ORIGINAL mtext(lab.gene.set[i], side=1, line=0, cex=gene.set.cex)		
      if(!is.null(gene.sets.results)){
        p.value.text <- gene.sets.results$FWER.p.val[match(lab.gene.set[i], gene.sets.results$NAME)]
        p.value.text <- paste(lab.gene.set[i], "; FWER=", p.value.text)
        mtext(p.value.text, side=1, line=1, cex=gene.set.cex)  
        
      } else {
        mtext(lab.gene.set[i], side=1, line=1, cex=gene.set.cex)  
      }
    }
    ...
    for (j in 1:nrow(enrich)) {
      ...
      in.edge <- enrich[j, 8]=="Yes"
      if(!in.edge | !edge.highlight){
        lines(c(index, index), c(0, enrich.score), lwd=gene.set.lwd, col=all.color)
      } else {
        lines(c(index, index), c(0, enrich.score), lwd=gene.set.lwd, col=edge.color)
      }
    }
  }
}
